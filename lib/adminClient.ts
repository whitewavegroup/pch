...
export async function listSponsors() { return []; }
...
export async function createReview(input: {
  ...
  const author =
    (await prisma.user.findUnique({ where: { email: input.authorEmail } })) ||
    (await prisma.user.create({ data: { email: input.authorEmail, role: 'CLIENT' } }));
