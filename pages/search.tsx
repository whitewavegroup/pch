...
export default function SearchPage({lang,q,country,results}:{lang:'en'|'es',q:string,country:string,results:any[]}){
  const i = t(lang);
  return (<Layout lang={lang}>
    <section className="search-hero">
      <h1 className="title">{i.results}</h1>
      <SearchBar placeholder={i.searchPlaceholder} initialQ={q} initialCountry={country} />
      <div className="meta" style={{marginTop:'.6rem'}}>{q && <>Query: <b>{q}</b> • </>}{country && <>Country: <b>{country}</b></>}</div>
    </section>
    {results.length===0 ? <p>{i.noResults}</p> : <div className="grid">{results.map(p=> <ProviderCard key={p.id} p={p} />)}</div>}
  </Layout>)
}
...
export const getServerSideProps: GetServerSideProps = async (ctx)=>{
  ...
  const results = await prisma.provider.findMany({ where, include:{ services:{ include:{ service:true } }, regions:true }, take:50, orderBy:[{updatedAt:'desc'}] });
